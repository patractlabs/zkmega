...
pub fn  alt_bn128_add(input: &[u8]) -> Result<[u8;64], &'static str> {
    ...
    if let Some(sum) = AffineG1::from_jacobian(p1 + p2) {
        ...
        sum.x().to_big_endian(&mut write_buf[0..32]).expect("Cannot success since 0..32 is 32-byte length");
        sum.y().to_big_endian(&mut write_buf[32..64]).expect("Cannot success since 32..64 is 32-byte length");
    }
...
fn add(){
    ...
    let x = U256([193057075356696845107056778628997597259, 58302148128148680438974674437016463407]);
    let y = U256([97531101576300296613937243006504225138, 35484485583128177644927852191793807725]);
    let z = U256([173294647386577742778337618982106380048, 54444350454913557390070431662659890840]);
    let p1_2times=  alt_bn128_scalar_mul(&hex!("0230644e72e131a029b85045b68181585d97816a916871ca8d3c208c16d87cfd46")[..]);
    println!("{:?}",p1_2times  );
