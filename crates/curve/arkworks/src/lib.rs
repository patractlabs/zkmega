...
/// Vector Addition
#[cfg(not(feature = "ink"))]
pub fn add(curve_id: u32, input: &[u8]) -> Result<Vec<u8>> {
    Ok(match curve_id {
...
/// Vector Addition
#[cfg(feature = "ink")]
pub fn add(curve_id: u32, input: &[u8]) -> Result<bool> {
    Ok(ink_env::call_chain_extension(curve_id, &Vec::from(input))?)
}

/// Scalar MulAssign
#[cfg(not(feature = "ink"))]
pub fn mul(curve_id: u32, input: &[u8]) -> Result<Vec<u8>> {
    Ok(match curve_id {
...
/// Scalar MulAssign
#[cfg(feature = "ink")]
pub fn mul(curve_id: u32, input: &[u8]) -> Result<bool> {
    Ok(ink_env::call_chain_extension(
        curve_id + 0x10,
        &Vec::from(input),
    )?)
}
...
/// Pairing
#[cfg(not(feature = "ink"))]
pub fn pairing(curve_id: u32, input: &[u8]) -> Result<bool> {
    Ok(match curve_id {
